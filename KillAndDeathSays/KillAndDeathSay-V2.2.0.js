...
function main() {
    initUI();
    Global.RegisterCallback("Draw", "updateUI");
...
function initUI() {
    ...
    UI.SetEnabled("Script items", "=======KillSay=======", false);
    UI.SetEnabled("Script items", "KillSay Type", false);
    UI.SetEnabled("Script items", "Kill: Include Victim Name", false)
    UI.SetEnabled("Script items", "Custom KillSay", false)

    UI.SetEnabled("Script items", "=======DeathSay======", false)
    UI.SetEnabled("Script items", "DeathSay Type", false)
    UI.SetEnabled("Script items", "Death: Include Attacker Name", false)
    UI.SetEnabled("Script items", "Custom DeathSay", false)
...
function updateUI() {
    ...
    updateMode();
    var killSayCustom = 0;
    var deathSayCustom = 0;
    if (get.state("KillSay Type") == 1 && mode[2]) {
        killSayCustom = 1;
    } else {
        killSayCustom = 0;
    }
    if (get.state("DeathSay Type") == 1 && mode[1]) {
        deathSayCustom = 1;
    } else {
        deathSayCustom = 0;
    }
    UI.SetEnabled("Script items", "=======KillSay=======", mode[2]);
    UI.SetEnabled("Script items", "KillSay Type", mode[2]);
    UI.SetEnabled("Script items", "Kill: Include Victim Name", mode[2]);
    UI.SetEnabled("Script items", "Custom KillSay", killSayCustom);

    UI.SetEnabled("Script items", "=======DeathSay======", mode[1]);
    UI.SetEnabled("Script items", "DeathSay Type", mode[1]);
    UI.SetEnabled("Script items", "Death: Include Attacker Name", mode[1]);
    UI.SetEnabled("Script items", "Custom DeathSay", deathSayCustom);
}

function updateMode() {
    mode = get.state("Select Mode(s)").toString(2).split("").reverse().map(Number);
}
...
function modeCheck() {
    ...
    if (mode[4]) {
        if (Entity.IsLocalPlayer(getPlayerID("attacker") && getPlayerID("victim"))) {
            Global.ExecuteCommand("say I felt bad for you guys, so I decided to kill myself for you.");
        }
    }
}
