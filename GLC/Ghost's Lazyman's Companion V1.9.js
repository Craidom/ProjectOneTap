/*
GhostDaSnipa's Lazyman Companion V1.9
updated 22 January 2020 09:11 PST
*/
...
var menu = {
    init() {
        ...
        UI.AddDropdown("IHY: K.O.T. Hitbox Selection", [
            "Head",
            "Neck",
            "Pelvis",
            "Body",
    ...
    update: {
        ...
        iHateYou(state) {
            ...
            UI.SetEnabled("IHY: K.O.T. Hitbox Selection", state);
...
var iHateYou = {
    ...
    rage() {
        ...
        var meEyePos = Entity.GetEyePosition(meID);
        var bulletDMG = null;
        var hitboxSelect = get.ui.value("IHY: K.O.T. Hitbox Selection");
        for (var i = 0; i < playerID.length; i++) {
            var headPos = Entity.GetHitboxPosition(playerID[i], hitboxSelect);
            var bulletDMG = Trace.Bullet(playerID[i], meID, meEyePos, headPos);
            ...
            if (!rage) {
                ...
                if (!Entity.IsAlive(playerID[i]) || Entity.IsDormant(playerID[i])) continue;
                if (get.ui.value("IHY: KillOnlyTarget")) {
                    iHateYou.killOnlyTarget(playerID[i], minDMG, meEyePos, hitboxSelect);
                    UI.SetValue("Legit", "General", "Enabled", true);
                    UI.ToggleHotkey("Legit", "Triggerbot", "Enabled"); //Triggerbot needs to be set to toggle instead of Hold or Always.
                    continue;
                } else {
    ...
    killOnlyTarget(playerID, minDMG, myEyePos, hitbox) {
